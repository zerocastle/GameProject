...
public class DoubleBuffering extends JFrame implements Runnable {
	...
	int enemyY = 50; int[] enemyXList = new int[15];
							// 여기까지 적 만들기 ==============
	...
	public void moveEn() {
		...
		if (returnPosition) {
			enemyY = enemyY + (int) (Math.random() * 3); //떨어지는 속도 ..컨트롤
			if(enemyY > HEIGHT) {
...
//		g.drawImage(enemyClass.enemyList.get(0), enemyClass.getX() + 1 , enemyClass.getY(),this); //적 그려주기 ...
//		System.out.println(enemyClass.getX() + "  " + enemyClass.getY());
		...
		if (rectX - circleSize < x && x < rectX + rectSize && rectY - circleSize < y && y < rectY + rectSize) {
		 ...
		 // 적의 x 축과 적의 가로 세로 크기 와 코인의 충돌 비교 판단
		for(int i = 0; i < enemyXList.length; i++) {
			if(this.enemyXList[i] - circleSize < x && x < this.enemyXList[i] + rectSize && enemyY - circleSize < y && y < enemyY + rectSize) {
		...
		// 아직 승리하지 않은 경우에 또는 적에게 안잡아먹힌 경우
		if (won == false)
			repaint();
